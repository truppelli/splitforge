...
| A journal write fails | The read path retries the same read, from 100 ms doubling to 5 s, for as long as it takes. It never moves past a read it could still store, and `/health` is degraded with the reason until a write lands. A read holding a value the schema cannot represent is refused before either file is written, set aside on the audit trail as `journal.unstorable`, and counted, and recording continues ([ADR-0031](adr/0031-a-failed-write-is-retried-and-an-unstorable-read-is-set-aside.md)) |
