...
| ~~A privacy promise checked against bytes~~ | ~~M5~~ | **Done.** `crates/splitforge-cli/tests/bundle.rs` runs a full event, takes a diagnostic bundle, and searches the raw file for every name, bib, chip, operator, and typed sentence the event contained. Asserting over the parsed JSON would only cover the fields somebody thought to check |
