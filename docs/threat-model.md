...
| O4 | **Database corruption** | Critical | Integrity check in `splitforge doctor`; pre-race snapshot; append-only journal limits blast radius. **OPEN:** [Q7](open-questions.md#q7-corruption-recovery-strategy) | M5 |
...
| O9 | **Accidental event-data deletion** | Critical | Destructive CLI commands require explicit confirmation naming the event; automatic pre-race snapshot; no `DELETE` path for `raw_reads` at all. **OPEN:** [Q8](open-questions.md#q8-enforcing-append-only-in-sqlite) | M2, M5 |
